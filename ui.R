...
ui <- dashboardPage(
  skin = "blue",
  dashboardHeader(title = 'Cell Annotation Tool'),
  dashboardSidebar(
    sidebarMenu(
      id = "tab",
      ...
      menuItem("Download FCS", tabName = "download", icon = icon("download"))
    )
  ),
  dashboardBody(
  ...
  ,
      ...
      ,
         # XGBoost predictions
          box(width = 12 , status="info", title = 'STEP 2 - Predictions',
              ...
              # Choose files to annotate
              radioButtons("FilesToAnnotate", "Files to annotate :", choices = list("")),
              
          uiOutput("file_checkboxes"),
          textOutput("messageXGBoost"),
    ...
    conditionalPanel(
      ...
      box(width = 10,status = "primary",
        ...
        box(width = 10 , status="info",title='STEP 1 - Load or build scyan knowledge table',
            ...
            fileInput("knowledgeTable", "Upload knowledge Table (CSV format) :", multiple = FALSE, buttonLabel = "Upload...", accept = c('.csv')),
    ...
    conditionalPanel(
      ...
      # actionButton("downloadSelectedCsvResult", "Download CSV results of selected file"),
      actionButton("downloadAllCsvResult", "Download ALL CSV results", style="background-color: #B2DFEE ; border-width: 2px")
      ),
      textOutput("download")
